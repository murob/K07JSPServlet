...
String pageTemp = request.getParameter("pageNum");//만약 파라미터로 전달된 페이지번호가 있다면 ...
...
<body>
	...
	<table border="1" width="90%">
		...
		else{//컬렉션에 저장된 데이터가 있다면 해당 내용을 반복하여 출력한다.
			int vNum = 0;//목록의 가상번호로 사용
			int countNum = 0;
			for(BoardDTO dto : boardLists)//List컬렉션에 저장된 갯수만큼 반복하기 위해 확장for문 사용
			{
...
<%--             <a href="View.jsp?num<%=">dto.getNum()%>&<%=qeuryStr %> "><%=dto.getTitle() %></a> --%>
